...
def addMultilAttrParams(encCfgPath,attrParaPath):
    params=open(attrParaPath,'r',encoding='UTF-8')
    file_write_enc=open(encCfgPath,'a',encoding='UTF-8')
    lines=params.readlines()
    for line in lines:
        file_write_enc.writelines(line)
        # file_write_enc.writelines('\n')
...
def gen_cfg(condition,sequences_filename,folder_name):
    ...
    f_condition = open('condition_multil.yaml')
    data = ordered_yaml_load('condition_multil.yaml')
    ...
    for seq_name in seqs:
        ...
        for r in rps_attr:
            ...
            # added the multil attribute params
            addMultilAttrParams('{}/encoder.cfg'.format(dir),'{}_multil_attr_params.txt'.format(seq_name))
            # decoder cfg
...
if __name__ == "__main__":
    folder_name="cfg_transform_multil"
    sequences_filename = "sequences_transform_tw_multil.yaml"
    gen_cfg("C1-limitlossyG-lossyA-ai",sequences_filename,folder_name)
    gen_cfg("C2-losslessG-lossyA-ai",sequences_filename,folder_name)
    gen_cfg("C3-losslessG-limitlossyA-ai",sequences_filename,folder_name)
    gen_cfg("C4-losslessG-losslessA-ai",sequences_filename,folder_name)
    
    folder_name="cfg_predict_multil"
    sequences_filename = "sequences_predict_tw_multil.yaml"
    gen_cfg("C3-losslessG-limitlossyA-ai",sequences_filename,folder_name)
    gen_cfg("C4-losslessG-losslessA-ai",sequences_filename,folder_name)
    
    folder_name="cfg_predict_multil"
    sequences_filename = "sequences_predtrans_limited_tw_multil.yaml"
    gen_cfg("C1-limitlossyG-lossyA-ai",sequences_filename,folder_name)
    gen_cfg("C2-losslessG-lossyA-ai",sequences_filename,folder_name)
